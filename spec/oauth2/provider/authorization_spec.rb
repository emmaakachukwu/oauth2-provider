...
describe OAuth2::Provider::Authorization do
  ...
  before do
    ...
    allow(OAuth2).to receive(:random_string).and_return('s1', 's2', 's3')
  end
  ...
  describe "#grant_access!" do
    ...
    describe "for token requests" do
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eq( @client)
        expect(authorization.code).to be_nil
        expect(authorization.access_token_hash).to eq(OAuth2.hashify("s1"))
        expect(authorization.refresh_token_hash).to eq(OAuth2.hashify("s2"))
      end
    end
    ...
    describe "for code_and_token requests" do
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eq(@client)
        expect(authorization.code).to eq("s1")
        expect(authorization.access_token_hash).to eq(OAuth2.hashify("s2"))
        expect(authorization.refresh_token_hash).to eq(OAuth2.hashify("s3"))
      end
    end
  end
